...
if not os.path.exists("C:\\Users\\"+username+"\\Pictures\\Steam game cache\\"):
    os.chdir("C:\\Users\\"+username+"\\Pictures")
    os.mkdir("Steam game cache")
img_path = "C:\\Users\\"+username+"\\Pictures\\Steam game cache\\"

## User configuration
user = input("Steam custom URL or profile ID (e.g. 'alifeee' or '5164879451874'):\n")
max_images = int(input("Number of games to display (0 for all):\n"))
while True:
    desired_ratio = input("Aspect Ratio: a)1:1 b)2:1 c)1:2 d)4:3 e)16:9\nor choose a custom aspect ratio in the format n:m\n") 
    if desired_ratio=='a':
        aspect_ratio = (1,1);break
    elif desired_ratio=='b':
        aspect_ratio = (2,1);break
    elif desired_ratio=='c':
        aspect_ratio = (1,2);break
    elif desired_ratio=='d':
        aspect_ratio = (4,3);break
    elif desired_ratio=='e':
        aspect_ratio = (16,9);break
    else:
        rats = re.findall("\d+", desired_ratio)
        if len(rats) != 2:
            print("Your aspect ratio was not understood. Please enter one in the format 'n:m'")
            continue
...
for i, game in enumerate(games):
    ...
    try:
        ...
        print("({})Cache not found! Retrieving from URL: {}".format(i,game['name']))
        try:
...
for i, img in enumerate(images):
    if i >= num_images:
        break
    box = ( (184*(i%n)), (69*((int(i/n))%m)), 184+(184*(i%n)), 69+(69*((int(i/n))%m)))
    thumb = img.crop(thumb_box)
    master_image.paste(thumb, box)

## Save the image to cache and preview it
input("Image saved to {}.".format(img_path+"users\\"+user+".png"))
master_image.save(img_path+"users\\"+user+".png")
preview = Image.open(img_path+"users\\"+user+".png")
preview.show()
